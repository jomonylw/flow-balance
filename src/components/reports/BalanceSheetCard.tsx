...
export default function BalanceSheetCard() {
  ...
  const enrichedCategoryTree = useMemo(() => {
    ...
    const buildCategoryTree = (type: 'ASSET' | 'LIABILITY') => {
      // 获取该类型的原始分类数据
      const rawCategories = data.balanceSheet[type === 'ASSET' ? 'assets' : 'liabilities'].categories
          ...
          {category.totalInBaseCurrency !== undefined && category.totalInBaseCurrency !== 0 && (
            <div className="text-right">
              <span className={`inline-block px-2 py-1 rounded text-sm font-bold border ${
                ...
                {baseCurrency.symbol}{Math.abs(category.totalInBaseCurrency).toLocaleString(language === 'zh' ? 'zh-CN' : 'en-US', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2
                })}
              </span>
                    ...
                    <div className="text-right min-w-0 flex-shrink-0">
                      ...
                      {currencyCode !== baseCurrency.code && (
                        <div className="text-xs text-gray-400 whitespace-nowrap">
                          ≈ {baseCurrency.symbol}{currencyAccounts.reduce((sum, account) =>
        ...
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div className="mb-6">
                {enrichedCategoryTree?.assets && enrichedCategoryTree.assets.length > 0 ? (
                  ...
                  <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-600">
                    <div className="flex justify-between font-semibold text-base">
                      <span className="text-gray-700 dark:text-gray-300">
                        {t('reports.balance.sheet.base.currency.total')} ({data.baseCurrency.code})
                      </span>
                      <div className="text-gray-900 dark:text-gray-100">
                        {formatCurrency(data.summary.baseCurrencyTotals.totalAssets, data.baseCurrency)}
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div>
              ...
              <div className="mb-6">
                {enrichedCategoryTree?.liabilities && enrichedCategoryTree.liabilities.length > 0 ? (
                  renderHierarchicalCategories(enrichedCategoryTree.liabilities)
                ) : (
                  renderCategorySection(
                    '',
                ...
                {data.summary.baseCurrencyTotals && (
                  <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-600">
                    <div className="flex justify-between font-semibold text-base">
                      ...
                      <div className="text-red-600 dark:text-red-400">
                        {formatCurrency(data.summary.baseCurrencyTotals.totalLiabilities, data.baseCurrency)}
                      </div>
                    </div>
                  </div>
                )}
                ...
                {data.summary.baseCurrencyTotals && (
                  <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-600">
                    <div className="flex justify-between font-semibold text-base">
                      ...
                      <div className={data.summary.baseCurrencyTotals.netWorth >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
                        {formatCurrency(data.summary.baseCurrencyTotals.netWorth, data.baseCurrency)}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('reports.balance.sheet.total.assets.summary')} ({data.baseCurrency.code})
                    </span>
                    <div className="font-semibold text-gray-900 dark:text-gray-100">
                      {formatCurrency(data.summary.baseCurrencyTotals.totalAssets, data.baseCurrency)}
                    </div>
                  ...
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('reports.balance.sheet.total.liabilities.summary')} ({data.baseCurrency.code})
                    </span>
                    <div className="font-semibold text-red-600 dark:text-red-400">
                      {formatCurrency(data.summary.baseCurrencyTotals.totalLiabilities, data.baseCurrency)}
                    </div>
                  ...
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('reports.balance.sheet.net.worth.summary')} ({data.baseCurrency.code})
                    </span>
                    <div className={`font-semibold ${data.summary.baseCurrencyTotals.netWorth >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {formatCurrency(data.summary.baseCurrencyTotals.netWorth, data.baseCurrency)}
                    </div>
                  </div>
                </div>
