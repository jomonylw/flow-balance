    ...
    <div className="w-80 flex-shrink-0">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden backdrop-blur-sm">
        {settingGroups.map((group, groupIndex) => (
          <div key={group.id} className={`${groupIndex > 0 ? 'border-t border-gray-100 dark:border-gray-700' : ''}`}>
            {/* 设置分组标题 */}
                    ...
                    {item.status && (
                      ...
                      `}>
                        ...
                        <span className="hidden sm:inline">
                          {item.status === 'complete' ? t('settings.status.complete') :
                           item.status === 'warning' ? t('settings.status.warning') :
                           item.status === 'incomplete' ? t('settings.status.incomplete') : ''}
                        </span>
                      </span>
                    )}
                  </div>
