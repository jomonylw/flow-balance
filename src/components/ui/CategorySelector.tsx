...
export default function CategorySelector({
  ...
  const processCategories = useCallback(() => {
    ...
    if (excludeCategoryId) {
      ...
      // 获取被排除分类的根分类类型
      const excludedCategory = allCategories.find(cat => cat.id === excludeCategoryId)
      if (excludedCategory) {
        // 转换为LocalCategory类型
        ...
        const excludedRootCategory = findRootCategory(excludedLocalCategory, categoriesData)
        if (excludedRootCategory?.type) {
          categoriesData = filterByAccountTypeFunc(categoriesData, excludedRootCategory.type)
        }
      }
    }
      ...
      <div className="space-y-4">
        <div className="max-h-96 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-md p-2 bg-white dark:bg-gray-800">
          {isLoading ? (
            ...
            <div className="text-center py-4 text-gray-500 dark:text-gray-400">{t('category.selector.no.categories')}</div>
          )}
        </div>
