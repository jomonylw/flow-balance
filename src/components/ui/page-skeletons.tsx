  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-2 border-b border-gray-200 dark:border-gray-700">
        {Array.from({ length: 3 }).map((_, i) => (
          <Skeleton key={i} height="2.5rem" width="120px" rounded="md" className="mb-2" />
        ))}
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="ml-4 space-y-2">
            {Array.from({ length: 6 }).map((_, i) => (
              <Skeleton key={i} height="1.5rem" width={`${70 + (i % 3) * 10}%`} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 sm:p-6">
        <div className="mb-4">
          <div className="flex items-center mb-2">
            <Skeleton width="20px" height="20px" rounded="sm" className="mr-2" />
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 sm:p-6">
        ...
        <div className="space-y-4">
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i}>
              <Skeleton height="1rem" width={`${60 + i * 10}px`} className="mb-2" />
              <Skeleton height="2.5rem" rounded="md" />
              {i === 1 && <Skeleton height="0.75rem" width="160px" className="mt-1" />}
            </div>
          ))}
 ...
 * 设置页面骨架屏（响应式）
 */
export function SettingsPageSkeleton({ isMobile = false }: { isMobile?: boolean }) {
  return isMobile ? <SettingsPageMobileSkeleton /> : <SettingsPageDesktopSkeleton />
}
