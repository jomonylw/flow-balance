...
export default function NorthStarMetrics({ params, currency }: NorthStarMetricsProps) {
  const { t } = useLanguage()
  ...
  if (monthlyRate > 0 && monthlyPayment > 0) {
    // NPER 计算公式
    const numerator = Math.log((futureValue * monthlyRate + monthlyPayment) / (presentValue * monthlyRate + monthlyPayment))
  ...
  // 当前进度
  const currentProgress = (params.currentInvestableAssets / fireTargetAmount) * 100
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900 dark:to-green-800 rounded-lg p-6 text-center">
          ...
          <p className="text-sm text-green-700 dark:text-green-300">
            {t('fire.north.star.current.progress.description', {
              amount: formatCurrency(params.currentInvestableAssets, currency.code)
            })}
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900 dark:to-purple-800 rounded-lg p-6 text-center">
          ...
          <div className="text-4xl font-bold text-purple-900 dark:text-purple-100 mb-2">
            ...
            <span className="text-lg font-normal">{t('fire.units.per.month')}</span>
          </div>
