...
import type { Currency, ConversionResult } from '@/types/core'
...
export default function CurrencyConversionStatus({
  ...
  return (
    <div
      className={`rounded-lg shadow p-4 ${className} ${resolvedTheme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}
    >
      <div className='flex items-center justify-between mb-3'>
        <h3
          className={`text-sm font-medium ${resolvedTheme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}
        >
        ...
        <div className='flex items-center space-x-2'>
          ...
          >
            {t('currency.base.currency')}: {baseCurrency.symbol}{' '}
            {baseCurrency.name}
          </span>
        </div>
      </div>
        ...
        {successfulConversions.length > 0 && (
          <div>
            <div className='text-xs font-medium text-green-700 mb-1'>
              ✅ {t('currency.conversion.successful')} (
            ...
            <div className='space-y-1'>
              {successfulConversions.slice(0, 3).map((conversion, index) => (
                ...
                >
                  ...
                  <span className='font-mono'>
                    1:
                    {conversion.exchangeRate.toLocaleString('zh-CN', {
              ...
              {successfulConversions.length > 3 && (
                <div className='text-xs text-gray-500'>
                  {t('currency.conversion.more.successful', {
                    count: successfulConversions.length - 3,
                  })}
                </div>
              )}
            </div>
            ...
            <div className='space-y-1'>
                ...
                <div key={index} className='text-xs text-red-600'>
                  <div className='flex items-center justify-between'>
                    ...
                    <span className='text-red-500'>
                      {t('currency.conversion.missing.rate')}
                    </span>
                  </div>
                  {conversion.error && (
              ...
              {failedConversions.length > 3 && (
                <div className='text-xs text-red-500'>
                  {t('currency.conversion.more.failed', {
                    count: failedConversions.length - 3,
                  })}
                </div>
              )}
            </div>
