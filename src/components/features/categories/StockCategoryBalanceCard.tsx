...
import type { BalanceStockAccount } from '@/types/components'
...
export default function StockCategoryBalanceCard({
    ...
    const fetchBalanceData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          accounts.forEach((account: BalanceStockAccount) => {
            Object.entries(account.balances).forEach(([currency, balance]) => {
              const balanceAmount = balance as number
              if (!currencyBreakdown[currency]) {
                currencyBreakdown[currency] = 0
              }
              currencyBreakdown[currency] += balanceAmount
  ...
  if (!balanceData) {
    return (
      <div className='bg-white shadow rounded-lg p-6'>
        <div className='text-center text-gray-500'>无法加载余额数据</div>
      </div>
    )
  }
    ...
    <div className='bg-white shadow rounded-lg p-6'>
      ...
      <div className='mb-6 p-4 bg-gray-50 rounded-lg'>
        <div className='text-center'>
          ...
          >
            {currencySymbol}
            {Math.abs(totalBalance).toLocaleString('zh-CN', {
            ...
            >
              <div className='flex justify-between items-start'>
                ...
                <div className='text-right'>
                  {Object.entries(account.balances).map(
                    ([currency, balance]) => (
                      <div key={currency} className='text-sm'>
                        ...
                        >
                          ...
                          {balance < 0 && (
                            <span className='text-xs ml-1'>(负)</span>
                          )}
                        </span>
                      </div>
                    ),
