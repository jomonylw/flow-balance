import type { SimpleCategory, SimpleTransaction } from '@/types/core'

// 本地类型定义（用于这个组件的特定统计需求）
interface CategorySummaryStats {
  totalAmount: number
  totalIncome: number
  totalExpense: number
...
export default function CategorySummaryCard({
  ...
  const latestTransaction =
    category.transactions.length > 0 ? category.transactions[0] : null
    ...
    <div className='bg-white shadow rounded-lg p-6'>
      <div className='grid grid-cols-1 md:grid-cols-4 gap-6'>
        ...
        <div className='text-center md:text-left'>
          <div className='text-sm font-medium text-gray-500 mb-1'>总金额</div>
          <div
          ...
          >
            {currencySymbol}
            {Math.abs(stats.totalAmount).toLocaleString('zh-CN', {
        ...
        {/* 本月金额 */}
        <div className='text-center md:text-left'>
          <div className='text-sm font-medium text-gray-500 mb-1'>本月金额</div>
          <div
          ...
          >
            ...
            {currencySymbol}
            {Math.abs(stats.thisMonthAmount).toLocaleString('zh-CN', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
          </div>
          {stats.monthlyChange !== 0 && (
            ...
            >
              {stats.monthlyChange > 0 ? '↗' : '↘'}{' '}
              {Math.abs(stats.monthlyChange).toLocaleString('zh-CN', {
        ...
        {/* 今年金额 */}
        <div className='text-center md:text-left'>
          <div className='text-sm font-medium text-gray-500 mb-1'>今年金额</div>
          <div
            className={`text-2xl font-semibold ${
              stats.thisYearAmount >= 0 ? 'text-blue-600' : 'text-red-600'
            }`}
          >
            {stats.thisYearAmount >= 0 ? '+' : ''}
            {currencySymbol}
            {Math.abs(stats.thisYearAmount).toLocaleString('zh-CN', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
          </div>
        ...
        {/* 平均金额 */}
        <div className='text-center md:text-left'>
          <div className='text-sm font-medium text-gray-500 mb-1'>平均金额</div>
          <div className='text-2xl font-semibold text-gray-900'>
            {currencySymbol}
            {averageAmount.toLocaleString('zh-CN', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
          </div>
          <div className='text-xs text-gray-500 mt-1'>每笔交易</div>
        </div>
      </div>
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-4 text-sm'>
          ...
          <div>
            ...
            <div className='font-medium text-gray-900'>
              {currencySymbol}
              {Math.abs(stats.lastMonthAmount).toLocaleString('zh-CN', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </div>
