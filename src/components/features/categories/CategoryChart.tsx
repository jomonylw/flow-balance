...
import type { TransactionType } from '@/types/core'
        ...
        formatter: function (params: unknown) {
          ...
          paramsArray.forEach((param: unknown) => {
            ...
            const formattedValue =
              value >= 0
                ? `+${currencySymbol}${value.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                : `-${currencySymbol}${Math.abs(value).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
            result += `${typedParam?.marker || ''}${typedParam?.seriesName || ''}: ${formattedValue}<br/>`
          })
          return result
        },
      },
