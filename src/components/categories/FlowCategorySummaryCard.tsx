...
// 直接使用 MonthlyReport[] 数组格式
type FlowSummaryData = {
  month: string
  childCategories: {
    id: string
    name: string
...
export default function FlowCategorySummaryCard({
  ...
  const calculateFlowStats = () => {
    ...
    const { monthNet: thisMonthNet, transactionCount: thisMonthTransactionCount } =
      latestMonthData ? calculateMonthlyNet(latestMonthData) : { monthNet: 0, transactionCount: 0 }
    ...
    // 计算月均（基于实际有数据的月份数）
    const thisYearMonthlyAverage = thisYearMonthCount > 0 ? thisYearNet / thisYearMonthCount : 0
    const lastYearMonthlyAverage = lastYearMonthCount > 0 ? lastYearNet / lastYearMonthCount : 0
  ...
  const calculateCurrencyDistribution = () => {
    ...
    // 过滤掉金额为0的币种
    const filteredStats = Object.entries(currencyStats).filter(([, data]) =>
    ...
    <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
          ...
          {accountType === 'INCOME' ? t('category.type.income') : t('category.type.expense')} • {t('category.type.flow')}
        </span>
      </div>
      ...
      {/* 币种分布 */}
      {summaryData && summaryData.length > 0 && Object.keys(currencyDistribution).length > 0 && (
        <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(currencyDistribution).map(([currencyCode, data]) => (
