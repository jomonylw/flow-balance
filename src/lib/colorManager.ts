...
export class ColorManager {
      ...
      if (colors[i] === '') {
        ...
        do {
          if (attempts < CHART_COLOR_SEQUENCE.length) {
            // 先尝试预定义颜色
            assignedColor = CHART_COLOR_SEQUENCE[defaultColorIndex % CHART_COLOR_SEQUENCE.length]
            defaultColorIndex++
          } else {
            // 如果预定义颜色不够，生成颜色变体
            const baseColorIndex = (attempts - CHART_COLOR_SEQUENCE.length) % CHART_COLOR_SEQUENCE.length
            const variantIndex = Math.floor((attempts - CHART_COLOR_SEQUENCE.length) / CHART_COLOR_SEQUENCE.length)
  ...
  static generateColorVariant(baseColor: string, variant: number): string {
    ...
    switch (variant % 6) {
      ...
      default:
        ...
        const newR = Math.max(0, Math.min(255, r + (adjustmentFactor * direction)))
        const newG = Math.max(0, Math.min(255, g + (adjustmentFactor * direction * 0.8)))
        const newB = Math.max(0, Math.min(255, b + (adjustmentFactor * direction * 0.6)))
   ...
   * @returns 调整后的颜色
   */
  static adjustColorHSL(color: string, hueShift: number, saturationShift: number, lightnessShift: number): string {
    const hex = color.replace('#', '')
