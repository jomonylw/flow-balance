    ...
    try {
      ...
      await prisma.exchangeRate.create({
        data: {
          ...
          notes: `自动生成的反向汇率，基于 ${rate.fromCurrencyRef?.code}→${rate.toCurrencyRef?.code}`,
        },
      })
...
async function generateTransitiveRates(userId: string, effectiveDate: Date) {
  ...
  try {
              ...
              if (!existing) {
                ...
                await prisma.exchangeRate.create({
                  data: {
                    ...
                    effectiveDate,
                    type: 'AUTO',
                    notes: `自动生成的传递汇率，计算路径: ${calculationPath}`,
                  },
                })
    ...
    result.errors.push(
      `传递汇率生成过程失败: ${error instanceof Error ? error.message : '未知错误'}`
    )
    return result
  }
}
...
export async function cleanupAutoGeneratedRates(
  ...
  try {
    ...
    result.errors.push(
      `清理自动生成汇率失败: ${error instanceof Error ? error.message : '未知错误'}`
    )
    return result
  }
}
