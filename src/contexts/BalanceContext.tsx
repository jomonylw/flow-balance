...
interface BalanceContextType {
  ...
  // 获取账户原始余额（指定货币）
  getAccountBalanceInCurrency: (accountId: string, currencyCode: string) => number | null
  // 获取分类汇总
  getCategorySummary: (categoryId: string) => CategorySummary | null
  // 更新单个账户余额（用于实时更新）
  updateAccountBalance: (accountId: string, balances: AccountBalance['balances']) => void
}
...
export function BalanceProvider({ children, categories, accounts }: BalanceProviderProps) {
  const [accountBalances, setAccountBalances] = useState<Record<string, AccountBalance>>({})
  const [baseCurrency, setBaseCurrency] = useState<{ code: string; symbol: string; name: string } | null>(null)
  ...
  const categorySummaries = React.useMemo(() => {
    ...
    // 递归计算分类汇总
    const calculateCategorySummary = (categoryId: string, currencyCode: string = 'CNY'): CategorySummary => {
      ...
      let currencySymbol = baseCurrency?.symbol || '¥'
  ...
  // 获取账户余额的便捷方法（本位币）
  const getAccountBalance = useCallback((accountId: string): number | null => {
  ...
  // 获取账户原始余额的便捷方法（指定货币）
  const getAccountBalanceInCurrency = useCallback((accountId: string, currencyCode: string): number | null => {
    const accountBalance = accountBalances[accountId]
  ...
  // 获取分类汇总的便捷方法
  const getCategorySummary = useCallback((categoryId: string): CategorySummary | null => {
    return categorySummaries[`${categoryId}-${baseCurrency?.code || 'CNY'}`] || null
  }, [categorySummaries, baseCurrency])

  // 更新单个账户余额
  const updateAccountBalance = useCallback((accountId: string, balances: AccountBalance['balances']) => {
