...
class DataUpdateManager {
  ...
  private async processEvent(event: DataUpdateEvent): Promise<void> {
    ...
    console.log(`[DataUpdateManager] Processing event ${event.type} for ${relevantListeners.length} listeners`)

    // 并行执行所有相关监听器
    const promises = relevantListeners.map(async listener => {
      try {
        console.log(`[DataUpdateManager] Calling listener ${listener.id} for event ${event.type}`)
...
// 便捷方法
export const publishDataUpdate = (event: DataUpdateEvent) => dataUpdateManager.publish(event)
