...
import { getMissingExchangeRates, getUserCurrencies } from '@/lib/currency-conversion'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const baseCurrency = userSettings?.baseCurrency || { code: 'USD', symbol: '$', name: 'US Dollar' }
    ...
    // 获取缺失的汇率
    const missingRates = await getMissingExchangeRates(user.id, baseCurrency.code)
    ...
    const allCurrencyPairs = userCurrencies
      ...
      .map(currency => {
        ...
        const hasRate = !missingRates.some(
          missing => missing.fromCurrency === currency && missing.toCurrency === baseCurrency.code
        )
