...
export const useAccountTransactions = (accountId: string, autoCheck: boolean = false) => {
  ...
  const checkTransactions = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/accounts/${accountId}/transactions?limit=1`)
      if (!response.ok) {
...
export const useBatchAccountTransactions = (accountIds: string[]) => {
  ...
  // 检查哪些账户需要获取数据
  const uncachedAccountIds = accountIds.filter(id => !(id in accountTransactionCache))
  ...
  const checkBatchTransactions = useCallback(async () => {
    ...
    try {
      ...
      // 更新缓存
      Object.entries(batchResults).forEach(([accountId, data]: [string, any]) => {
  ...
  // 获取所有账户的交易状态
  const getTransactionStatus = useCallback((accountId: string) => {
    return {
