...
async function testDashboardCalculations() {
  try {
    ...
    for (const account of stockAccounts.slice(0, 3)) {
      // 只测试前3个账户
    ...
    for (const account of flowAccounts.slice(0, 3)) {
      // 只测试前3个账户
      console.log(`\n  📊 账户: ${account.name} (${account.category?.type})`)
      console.log(`     交易数量: ${account.transactions.length}`)
    ...
    for (const category of topCategories.slice(0, 2)) {
      ...
      try {
        if (category.type === 'ASSET' || category.type === 'LIABILITY') {
          const stockSummary = await getStockCategorySummary(
            category.id,
            user.id
          )
          ...
          if (stockSummary.length > 0) {
            ...
            console.log(
              `       子分类数量: ${latestMonth.childCategories.length}`
            )
            console.log(
              `       直属账户数量: ${latestMonth.directAccounts.length}`
            )
          }
