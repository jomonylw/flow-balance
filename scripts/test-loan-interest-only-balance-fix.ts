...
async function testInterestOnlyBalanceGeneration() {
  ...
  try {
    ...
    console.log('\n还款计划:')
    calculation.schedule.forEach((payment, index) => {
    ...
    const loanContract = await LoanContractService.createLoanContract(testUser.id, contractData)
    ...
    for (let i = 0; i < Math.min(3, payments.length); i++) {
      ...
      console.log(`  剩余余额: ${Number(payment.remainingBalance).toLocaleString()}`)

      // 处理还款
      const success = await LoanContractService.processLoanPaymentRecord(payment.id)
      ...
      if (success) {
        ...
        if (updatedPayment) {
          ...
          if (updatedPayment.principalTransaction) {
            console.log(`    本金交易: ${updatedPayment.principalTransaction.amount} (${updatedPayment.principalTransaction.type})`)
          } else {
            console.log(`    本金交易: 无 (本金为0)`)
          }
          
          if (updatedPayment.interestTransaction) {
            console.log(`    利息交易: ${updatedPayment.interestTransaction.amount} (${updatedPayment.interestTransaction.type})`)
          } else {
            console.log(`    利息交易: 无`)
          }
          
          if (updatedPayment.balanceTransaction) {
            console.log(`    ✅ 余额交易: ${updatedPayment.balanceTransaction.amount} (${updatedPayment.balanceTransaction.type})`)
          } else {
    ...
    if (updatedLiabilityAccount) {
      console.log(`负债账户当前余额: ${Number(updatedLiabilityAccount.currentBalance).toLocaleString()}`)
    }
