...
async function fixCategoryTypes() {
  ...
  try {
    ...
    // 显示未能推断类型的分类
    const unidentifiedCategories = categories.filter(
      cat => !inferCategoryType(cat.name) && !cat.type
    )

    if (unidentifiedCategories.length > 0) {
      console.log(
        `⚠️  以下 ${unidentifiedCategories.length} 个分类无法自动推断类型，需要手动设置：\n`
      )
    ...
    console.log(
      `🎉 修正完成！现在有 ${updatedCategories.length} 个分类已设置类型`
    )
  } catch (error) {
...
async function showCurrentStatus() {
  ...
  Object.entries(typeGroups).forEach(([type, cats]) => {
    if (cats.length > 0) {
      ...
      console.log(
        `${typeNames[type as keyof typeof typeNames]} (${cats.length} 个):`
      )
