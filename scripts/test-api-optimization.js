...
async function testAPIOptimization() {
  ...
  try {
    ...
    if (treeResponse.ok) {
      ...
      console.log(
        `📋 获取到 ${treeData.data.stats.totalCategories} 个分类, ${treeData.data.stats.totalAccounts} 个账户`
      )
      ...
      const analyzeTree = (categories, level = 0) => {
        ...
        categories.forEach(category => {
          ...
          if (level === 0) {
            console.log(
              `  📂 ${category.name} (${category.type}): ${accountCount} 个账户`
            )
  ...
  try {
    ...
    if (balanceResponse.ok) {
      ...
      console.log(
        `✅ 账户余额API响应时间: ${balanceEndTime - balanceStartTime}ms`
      )
      ...
      console.log(
        `💵 总余额 (本位币): ${balanceData.data.baseCurrency.symbol}${totalBalance.toFixed(2)}`
      )
      console.log(`🌍 涉及货币:`)
      Object.entries(currencyStats).forEach(([currency, stats]) => {
        console.log(
          `  - ${currency}: ${stats.count} 个账户, 总额 ${stats.totalAmount.toFixed(2)}`
        )
      })
    } else {
    ...
    if (treeRes.ok && balanceRes.ok) {
        ...
        return categories.map(category => ({
          ...
          accounts:
            category.accounts?.map(account => ({
              ...account,
              balances:
                balanceData.data.accountBalances[account.id]?.balances || {},
              balanceInBaseCurrency:
                balanceData.data.accountBalances[account.id]
      ...
      const calculateCategorySummary = category => {
        ...
        category.children?.forEach(child => {
          ...
          Object.entries(childSummary.currencyBalances).forEach(
            ([currency, amount]) => {
              if (!currencyBalances[currency]) currencyBalances[currency] = 0
      ...
      enrichedTree.forEach(rootCategory => {
        ...
        console.log(
          `  📂 ${rootCategory.name}: ${balanceData.data.baseCurrency.symbol}${summary.totalBalance.toFixed(2)}`
        )

        Object.entries(summary.currencyBalances).forEach(
          ([currency, amount]) => {
