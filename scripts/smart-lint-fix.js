  ...
  try {
    ...
    SAFE_FIXES.convertToWarn.patterns.forEach(pattern => {
      const newContent = content.replace(
        pattern,
        SAFE_FIXES.convertToWarn.replacement
      )
      if (newContent !== content) {
        content = newContent
        modified = true
...
function main() {
  ...
  if (afterStats) {
    ...
    console.log(
      `  Console statements: ${afterStats.console} (${beforeStats.console - afterStats.console} fixed)`
    )
    console.log(
      `  Total: ${afterStats.total} (${beforeStats.total - afterStats.total} fixed)`
    )
