...
function removeDebugConsole() {
  ...
  consoleFiles.forEach((errors, filePath) => {
    try {
      ...
      // 从后往前处理，避免行号变化
      errors.sort((a, b) => b.line - a.line).forEach(error => {
...
function main() {
  ...
  // 获取初始统计
  const beforeCount = parseInt(execSync('pnpm lint 2>&1 | grep -E "(Warning|Error):" | wc -l', { encoding: 'utf8' }).trim());
  ...
  // 检查结果
  console.log('\n📊 Checking results...');
  const afterCount = parseInt(execSync('pnpm lint 2>&1 | grep -E "(Warning|Error):" | wc -l', { encoding: 'utf8' }).trim());
