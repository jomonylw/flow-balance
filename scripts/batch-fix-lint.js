  ...
  try {
    ...
    for (const line of lines) {
      if (line.startsWith('./src/')) {
        ...
        // 解析未使用变量错误
        const unusedVarMatch = line.match(/(\d+):(\d+)\s+Error:\s+'([^']+)'\s+is defined but never used/);
        if (unusedVarMatch) {
        ...
        // 解析空对象类型错误
        const emptyObjectMatch = line.match(/(\d+):(\d+)\s+Error:\s+An interface declaring no members/);
        if (emptyObjectMatch) {
        ...
        // 解析未使用表达式错误
        const unusedExprMatch = line.match(/(\d+):(\d+)\s+Error:\s+Expected an assignment or function call/);
        if (unusedExprMatch) {
...
function fixUnusedVariables(filePath, errors) {
  ...
  for (const error of sortedErrors) {
    ...
    if (lineIndex >= 0 && lineIndex < lines.length) {
      ...
      // 处理函数参数
        ...
        lines[lineIndex] = line.replace(new RegExp(`\\b${varName}:`), `_${varName}:`);
        modified = true;
      }
      // 处理变量声明
      else if (line.includes(`const ${varName} =`) || line.includes(`let ${varName} =`)) {
  ...
  if (modified) {
    ...
    const cleanedLines = lines.map(line => {
      ...
      // 删除空的导入行
      if (line.trim() === '' && lines[index] && lines[index].includes('import')) {
        return false;
      }
      return true;
    });
...
function fixEmptyObjectTypes(filePath, errors) {
  ...
  for (const error of errors) {
    ...
    if (lineIndex >= 0 && lineIndex < lines.length) {
      ...
      if (line.includes('interface') && line.includes('extends') && line.includes('{}')) {
        const interfaceMatch = line.match(/interface\s+(\w+)\s+extends\s+([^{]+)\s*{}/);
        if (interfaceMatch) {
          const [, interfaceName, baseType] = interfaceMatch;
          lines[lineIndex] = `type ${interfaceName} = ${baseType} & Record<string, never>`;
          modified = true;
        }
      }
    }
...
function main() {
  ...
  console.log('\n🔍 重新检查lint状态...');
  try {
    const newOutput = execSync('pnpm run lint 2>&1 | grep "Error:" | wc -l', { encoding: 'utf8' });
