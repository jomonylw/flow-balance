  ...
  for (const error of sortedErrors) {
    ...
    if (lineIndex >= 0 && lineIndex < lines.length) {
      ...
      if (line.includes(`function GET(${varName}:`)) {
        lines[lineIndex] = line.replace(`function GET(${varName}:`, `function GET(_${varName}:`);
        modified = true;
      } else if (line.includes(`function POST(${varName}:`)) {
        lines[lineIndex] = line.replace(`function POST(${varName}:`, `function POST(_${varName}:`);
        modified = true;
      } else if (line.includes(`function PUT(${varName}:`)) {
        lines[lineIndex] = line.replace(`function PUT(${varName}:`, `function PUT(_${varName}:`);
        modified = true;
      } else if (line.includes(`function DELETE(${varName}:`)) {
        lines[lineIndex] = line.replace(`function DELETE(${varName}:`, `function DELETE(_${varName}:`);
        modified = true;
        ...
        modified = true;
      } else if (line.includes(`${varName},`) && (line.includes('import') || line.includes('{'))) {
        // 处理导入中的未使用变量
        if (line.includes(`{ ${varName} }`)) {
          ...
          // 多个导入中的一个
          lines[lineIndex] = line.replace(`${varName},`, '').replace(`  ,`, '').replace(`, ,`, ',');
          modified = true;
        } else if (line.includes(`, ${varName}`)) {
