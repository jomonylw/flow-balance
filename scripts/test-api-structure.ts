...
async function testAPIStructure() {
  try {
    ...
    const _flowAccounts = accountsForCalculation.filter(
      account =>
    ...
    console.log('\n🧮 前端应该显示:')
    console.log(
      `总资产: ${apiResponse.data.totalAssets.currency.symbol}${apiResponse.data.totalAssets.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )
    console.log(
      `总负债: ${apiResponse.data.totalLiabilities.currency.symbol}${apiResponse.data.totalLiabilities.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )
    console.log(
      `净资产: ${apiResponse.data.netWorth.amount >= 0 ? '+' : ''}${apiResponse.data.netWorth.currency.symbol}${apiResponse.data.netWorth.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )
    ...
    console.log(
      `summaryData.totalAssets.amount: ${summaryData.totalAssets?.amount}`
    )
    console.log(
      `summaryData.totalAssets.currency: ${JSON.stringify(summaryData.totalAssets?.currency)}`
    )

    console.log(
      `summaryData.totalLiabilities 存在: ${!!summaryData.totalLiabilities}`
    )
    console.log(
      `summaryData.totalLiabilities.amount: ${summaryData.totalLiabilities?.amount}`
    )
    console.log(
      `summaryData.totalLiabilities.currency: ${JSON.stringify(summaryData.totalLiabilities?.currency)}`
    )
