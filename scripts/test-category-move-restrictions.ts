...
async function testCategoryMoveRestrictions() {
  try {
    ...
    topLevelCategories.forEach(topCategory => {
      console.log(`📂 ${topCategory.name} (${topCategory.type}) - ID: ${topCategory.id}`)
    ...
    if (assetCategory && expenseCategory) {
      ...
      try {
        const response = await fetch(`http://localhost:3001/api/categories/${assetCategory.id}`, {
          method: 'PUT',
          headers: {
    ...
    if (cashCategory && expenseCategory) {
      ...
      try {
        const response = await fetch(`http://localhost:3001/api/categories/${cashCategory.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    if (cashCategory && bankCategory) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // 恢复原状
          await fetch(`http://localhost:3001/api/categories/${cashCategory.id}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
