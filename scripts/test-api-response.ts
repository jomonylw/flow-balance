...
async function testAPIResponse() {
  try {
    ...
    console.log('净资产:')
    console.log(
      `  金额: ${apiResponse.netWorth.currency.symbol}${apiResponse.netWorth.amount.toFixed(2)}`
    )
    ...
    console.log('\n总资产:')
    console.log(
      `  金额: ${apiResponse.totalAssets.currency.symbol}${apiResponse.totalAssets.amount.toFixed(2)}`
    )
    ...
    console.log('\n总负债:')
    console.log(
      `  金额: ${apiResponse.totalLiabilities.currency.symbol}${apiResponse.totalLiabilities.amount.toFixed(2)}`
    )
    console.log(`  账户数量: ${apiResponse.totalLiabilities.accountCount}`)
    console.log(
      `  转换错误: ${apiResponse.totalLiabilities.hasConversionErrors}`
    )

    console.log('\n🧮 验证计算:')
    console.log(
      `  总资产 - 总负债 = ${apiResponse.totalAssets.amount.toFixed(2)} - ${apiResponse.totalLiabilities.amount.toFixed(2)} = ${(apiResponse.totalAssets.amount - apiResponse.totalLiabilities.amount).toFixed(2)}`
    )
    console.log(`  净资产 = ${apiResponse.netWorth.amount.toFixed(2)}`)

    const calculationMatch =
      Math.abs(
        apiResponse.totalAssets.amount -
          apiResponse.totalLiabilities.amount -
          apiResponse.netWorth.amount
      ) < 0.01
    console.log(`  计算一致性: ${calculationMatch ? '✅ 正确' : '❌ 错误'}`)

    console.log('\n📋 前端显示预期:')
    console.log(
      `  总资产: ${apiResponse.totalAssets.currency.symbol}${apiResponse.totalAssets.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )
    console.log(
      `  总负债: ${apiResponse.totalLiabilities.currency.symbol}${apiResponse.totalLiabilities.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )
    console.log(
      `  净资产: ${apiResponse.netWorth.amount >= 0 ? '+' : ''}${apiResponse.netWorth.currency.symbol}${apiResponse.netWorth.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )
