...
async function validateDashboardValues() {
  try {
    ...
    const liabilityAccounts = accounts.filter(
      acc => acc.category?.type === 'LIABILITY'
    )
    ...
    for (const account of assetAccounts) {
      ...
      const otherTransactions = account.transactions.filter(
        t => t.type !== 'BALANCE'
      )
      ...
      if (balanceAdjustments.length > 0) {
        ...
        console.log(
          `     - 最新余额调整: ${latest.date.toISOString().split('T')[0]} ${parseFloat(latest.amount.toString()).toFixed(2)}`
        )
      }
    ...
    console.log(
      `净资产 (估算): ¥${(totalAssets - totalLiabilities).toFixed(2)}`
    )
