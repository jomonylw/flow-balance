  ...
  try {
    ...
    for (const line of lines) {
      if (line.startsWith('./src/')) {
        currentFile = line;
      } else if (line.includes('Error:') && line.includes('is defined but never used') && currentFile) {
        const match = line.match(/(\d+):(\d+)\s+Error:\s+'([^']+)'\s+is defined but never used/);
        if (match) {
...
function main() {
  ...
  // 显示总体错误数量
  try {
    const totalOutput = execSync('pnpm run lint 2>&1 | grep "Error:" | wc -l', { encoding: 'utf8' });
