...
function simulateTransactionTypeMatching() {
  ...
  const testCases = [
    {
      accountType: 'INCOME',
      expectedType: 'INCOME',
      description: '收入账户应匹配收入交易',
    },
    {
      accountType: 'EXPENSE',
      expectedType: 'EXPENSE',
      description: '支出账户应匹配支出交易',
    },
  ]
  ...
  testCases.forEach((testCase, index) => {
    ...
    if (isValid) {
      console.log(
        `✅ 测试通过: ${testCase.accountType} 账户匹配 ${testCase.expectedType} 交易`
      )
    } else {
      console.log(
        `❌ 测试失败: ${testCase.accountType} 账户不匹配 ${testCase.expectedType} 交易`
      )
    }
  })
}
...
function checkConsoleLogging() {
  ...
  console.log(
    'Getting available transaction types for account:',
    'Test Account',
    'type:',
    'INCOME'
  )
...
window.testTransactionTypeMatching = function (accountType, transactionType) {
  console.log(
    `🧪 手动测试: 账户类型=${accountType}, 交易类型=${transactionType}`
  )
...
console.log('')
console.log(
  '💡 提示: 可以使用 testTransactionTypeMatching("INCOME", "INCOME") 进行手动测试'
)
