...
function simulateCurrencyValidation() {
  ...
  testCases.forEach((testCase, index) => {
    ...
    if (result) {
      console.log(`✅ 测试通过: ${testCase.accountCurrency} 账户 ${isValid ? '可以' : '不能'} 使用 ${testCase.transactionCurrency} 货币`);
    } else {
      console.log(`❌ 测试失败: 预期 ${testCase.expected ? '通过' : '失败'}，实际 ${isValid ? '通过' : '失败'}`);
    }
  });
}
...
function checkConsoleLogging() {
  ...
  // 模拟调试日志
  console.log('Account currency restriction effect - Account currency: CNY, Current form currency: USD');
...
function createManualTestFunctions() {
  // 手动测试货币匹配
  window.testCurrencyMatching = function(accountCurrency, transactionCurrency) {
    console.log(`🧪 手动测试: 账户货币=${accountCurrency}, 交易货币=${transactionCurrency}`);
    
    if (accountCurrency && accountCurrency !== transactionCurrency) {
      console.error(`❌ 验证失败: 此账户只能使用 ${accountCurrency}，无法使用 ${transactionCurrency}`);
      return false;
    }
