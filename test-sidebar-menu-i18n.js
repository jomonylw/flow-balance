...
function testSidebarMenuInternationalization() {
  ...
  const languageToggle =
    document.querySelector('[data-testid="language-toggle"]') ||
  ...
  console.log(
    `📊 找到 ${categoryItems.length} 个分类项和 ${accountItems.length} 个账户项`
  )
    ...
    return new Promise(resolve => {
      ...
      setTimeout(() => {
        ...
        if (contextMenu) {
          ...
          // 检查菜单项文本
          const menuItems = contextMenu.querySelectorAll(
            'button, [role="menuitem"]'
          )
          const menuTexts = Array.from(menuItems)
          ...
          if (currentLang === 'en') {
            ...
            const hardcodedChinese = menuTexts.filter(text =>
